...
testthat::test_that("client rejects stale publications and exposes semantic readiness", {
  ...
  testthat::expect_match(source, "shinycapAuthoritativeNodeCount", fixed = TRUE)
  testthat::expect_match(source, "shinycapAuthoritativeEdgeCount", fixed = TRUE)
  testthat::expect_match(source, "setEdges(next.edges)", fixed = TRUE)
  testthat::expect_match(source, "useUpdateNodeInternals", fixed = TRUE)
  testthat::expect_match(source, "protocol.current.lastPublication !== publication", fixed = TRUE)
  testthat::expect_match(source, "selected: node.id === selectedNodeId", fixed = TRUE)
