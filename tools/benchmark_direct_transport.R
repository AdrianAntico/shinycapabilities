...
rows <- lapply(c(100L, 1000L, 10000L), function(n) { items <- make_items(n); widget <- command_palette(items); direct <- command_palette_direct(items)
  widget_json <- jsonlite::toJSON(widget$x, auto_unbox = TRUE, null = "null", digits = NA)
  ...
  rbind(data.frame(n, transport = "htmlwidgets", t(measure(function() command_palette(items))), bytes = nchar(widget_json, type = "bytes")),
