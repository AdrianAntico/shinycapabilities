...
const themeProperties = ["bg", "panel", "text", "muted", "border", "accent", "hover", "invalid"];
const portalTheme = host => {
  const computed = window.getComputedStyle(host);
  return Object.fromEntries(themeProperties.map(name => {
    const property = `--sc-selection-${name}`;
...
function SelectionInput({ host, model }) {
  ...
  const invalid = model.required && !value.length;
  ...
  const popup = open && createPortal(<div ref={popupRef} className="sc-selection-popup" style={portalTheme(host)} data-selection-popup={host.id}
